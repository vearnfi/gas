...
function dataGas(data: string | undefined): number {
  ...
  const amountZeroBytes = bytes.reduce(
    (amountZeroBytes, byte) => amountZeroBytes + (byte === "00" ? 1 : 0),
    0,
  );
  const amountNonZeroBytes = bytes.length - amountZeroBytes;

  return (
    amountNonZeroBytes * NON_ZERO_BYTES_GAS + amountZeroBytes * ZERO_BYTES_GAS
  );
}
