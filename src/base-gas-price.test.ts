...
import {BaseGasPrice, makeBaseGasPrice} from "./base-gas-price";

describe("basGasPrice", () => {
  let baseGasPrice: BaseGasPrice;

  beforeEach(() => {
    baseGasPrice = makeBaseGasPrice("https://node.vechain.energy");
  });

  it("returns a BigNumber", async () => {
    const price = await baseGasPrice();
